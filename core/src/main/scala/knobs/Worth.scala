...
sealed abstract class Worth[A] extends Product with Serializable {
  def map[B](f: A => B): Worth[B]
  def worth: A
  def isRequired: Boolean
}

final case class Required[A](worth: A) extends Worth[A] {
  def map[B](f: A => B): Worth[B] =
    Required(f(worth))
  def isRequired = true
}

final case class Optional[A](worth: A) extends Worth[A] {
  def map[B](f: A => B): Worth[B] =
    Optional(f(worth))
